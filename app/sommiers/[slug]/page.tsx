...
import { sommierBySlugQuery, allSommierSlugsQuery } from "@/lib/sanity/product-queries";
...
export async function generateStaticParams() {
  ...
  try {
    const slugs = await sanityClient.fetch<{ slug: string }[]>(allSommierSlugsQuery);
...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  if (!sanityClient) return buildMetadata({ path: `/sommiers/${slug}` });
  const matches = await sanityClient.fetch<any[]>(sommierBySlugQuery, { slug }).catch(() => null);
  const resolved = resolveProductBySlug(matches, slug);
  if (!resolved) return buildMetadata({ path: `/sommiers/${slug}`, noindex: true });
  ...
  return buildMetadata({
    ...
    path: `/sommiers/${resolved.canonicalSlug}`,
    image,
    type: "product",
  });
}

export default async function SommierPage({ params }: { params: Promise<Params> }) {
  const { slug } = await params;
  if (!sanityClient) notFound();

  const [matches, siteSettings] = await Promise.all([
    sanityClient.fetch<any[]>(sommierBySlugQuery, { slug }).catch(() => null),
  ...
  if (resolved.shouldRedirect) permanentRedirect(productUrlFor("/sommiers", resolved.canonicalSlug));
  ...
  const breadcrumbs = [
    ...
    { name: product.name, url: `/sommiers/${resolved.canonicalSlug}` },
  ];
    ...
    <>
      ...
      <main className="mx-auto max-w-site px-6 py-10 md:px-10 md:py-14">
        ...
        <div className="mb-10 max-w-2xl md:mb-14">
          <span className="eyebrow-editorial on-cream mb-3">Sommier</span>
        ...
        <ProductPageSections product={product} basePath="/sommiers" />
      </main>
      ...
      <JsonLd
        data={faqSchema(
          product.productFaq?.length > 0
            ? product.productFaq
            : defaultFaq(product.productType || "sommier", product)
        )}
      />
      <JsonLd
        data={productSchema({
          ...
          sku: product.sku,
          brand: "DreamsFly",
          url: `/sommiers/${resolved.canonicalSlug}`,
          price: minPrice || 0,
          priceCurrency: "EUR",
          compareAtPrice: maxComparePrice > minPrice ? maxComparePrice : undefined,
