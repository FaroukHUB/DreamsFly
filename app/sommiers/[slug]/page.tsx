...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  const product = await sanityClient.fetch<any>(sommierBySlugQuery, { slug }).catch(() => null);
  if (!product) return buildMetadata({ path: `/sommiers/${slug}`, noindex: true });
...
export default async function SommierPage({ params }: { params: Promise<Params> }) {
  ...
  const breadcrumbs = [
    ...
    { name: product.name, url: `/sommiers/${slug}` },
  ];
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-site px-6 py-8 md:px-8 md:py-12">
        ...
        <div className="mb-8">
          <div className="mb-2 text-xs font-semibold uppercase tracking-widest text-pierre">Sommier</div>
