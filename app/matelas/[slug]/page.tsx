...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  const matches = await sanityClient.fetch<any[]>(productBySlugFullQuery, { slug }).catch(() => null);
  const resolved = resolveProductBySlug(matches, slug);
  if (!resolved) return buildMetadata({ path: `/matelas/${slug}`, noindex: true });
  ...
  return buildMetadata({
    ...
    path: `/matelas/${resolved.canonicalSlug}`,
    image,
    type: "product",
  });
}
...
export default async function ProductPage({ params }: { params: Promise<Params> }) {
  ...
  if (resolved.shouldRedirect) permanentRedirect(productUrlFor("/matelas", resolved.canonicalSlug));
  ...
  const breadcrumbs = [
    ...
    { name: product.name, url: `/matelas/${resolved.canonicalSlug}` },
  ];
      ...
      <main className="mx-auto max-w-site px-6 py-10 md:px-10 md:py-14">
        ...
        <div className="mb-10 max-w-2xl md:mb-14">
          <span className="eyebrow-editorial on-cream mb-3">
            Modèle {product.name || product.title}
          </span>
      ...
      <JsonLd
        data={faqSchema(
          product.productFaq?.length > 0
            ? product.productFaq
            : defaultFaq(product.productType || "matelas", product)
        )}
      />
      <JsonLd
        data={productSchema({
          ...
          sku: product.sku,
          brand: "DreamsFly",
          url: `/matelas/${resolved.canonicalSlug}`,
          price: minPrice || 0,
          priceCurrency: "EUR",
          compareAtPrice: maxComparePrice > minPrice ? maxComparePrice : undefined,
