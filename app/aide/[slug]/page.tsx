...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  if (!sanityClient) return buildMetadata({ path: `/aide/${slug}` });
  const p = await sanityClient.fetch<any>(staticPageBySectionSlugQuery, { section: "aide", slug }).catch(() => null);
  if (!p) return buildMetadata({ path: `/aide/${slug}`, noindex: true });
...
export default async function AidePage({ params }: { params: Promise<Params> }) {
  ...
  const [page, siteSettings] = await Promise.all([
    sanityClient.fetch<any>(staticPageBySectionSlugQuery, { section: "aide", slug }).catch(() => null),
  ...
  const breadcrumbs = [
    { name: "Accueil", url: "/" },
    { name: "Aide", url: "/aide/faq" },
    { name: page.title, url: `/aide/${slug}` },
  ];
