...
export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  ...
  const matches = await sanityClient.fetch<any[]>(oreillerBySlugQuery, { slug }).catch(() => null);
  const resolved = resolveProductBySlug(matches, slug);
  if (!resolved) return buildMetadata({ path: `/oreillers/${slug}`, noindex: true });
  ...
  return buildMetadata({
    ...
    path: `/oreillers/${resolved.canonicalSlug}`,
    image,
    type: "product",
  });
}
...
export default async function OreillerPage({ params }: { params: Promise<Params> }) {
  ...
  if (resolved.shouldRedirect) permanentRedirect(productUrlFor("/oreillers", resolved.canonicalSlug));
  ...
  const breadcrumbs = [
    ...
    { name: product.name, url: `/oreillers/${resolved.canonicalSlug}` },
  ];
    ...
    <>
      ...
      <JsonLd
        data={productSchema({
          ...
          sku: product.sku,
          brand: "DreamsFly",
          url: `/oreillers/${resolved.canonicalSlug}`,
          price: minPrice || 0,
          priceCurrency: "EUR",
          compareAtPrice: maxComparePrice > minPrice ? maxComparePrice : undefined,
