...
/**
 * Bande USP — 4 arguments clés sous le hero.
 * tone="cream" (défaut) : crème, icônes noires — sections intérieures
 * tone="noir"           : noir, icônes or, texte ivoire — sous le hero
 */
export function UspStrip({
  ...
  return (
    <div
      className={`border-y ${
        isNoir ? "border-white/10 bg-noir" : "border-ink/10 bg-creme"
      }`}
    >
      <div
        className={`mx-auto grid max-w-site divide-y md:grid-cols-4 md:divide-x md:divide-y-0 px-0 ${
          isNoir ? "divide-white/10" : "divide-ink/10"
        }`}
      >
        {usps.map((u, i) => (
          <div key={i} className="flex items-center gap-4 px-6 py-6 md:px-8 md:py-8">
            <div
              className={`flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full border ${
                isNoir ? "border-or/40 text-or" : "border-noir/20 text-noir"
              }`}
            >
            ...
            <div>
              <strong
                className={`block font-serif text-[15px] font-normal leading-tight md:text-[16px] ${
                  isNoir ? "text-ivoire" : "text-noir"
                }`}
              >
                {u.title}
              </strong>
              <span
                className={`mt-1 block font-sans text-[11px] uppercase tracking-[0.16em] ${
                  isNoir ? "text-or/80" : "text-taupe"
                }`}
              >
                {u.subtitle}
              </span>
